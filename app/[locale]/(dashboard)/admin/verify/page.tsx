...
import { adminVerifyAction } from "@/actions/admin-auth";
...
export default function AdminVerifyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await adminVerifyAction(password);
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#FAFAFA] text-[#0A0A0A] font-inter">
      <div className="w-full max-w-sm p-8 bg-white border border-[#E5E5E5]">
        <h1 className="text-2xl font-bold mb-6 text-center">Verify Identity</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Enter SECOND_ADMIN_PASS"
              required
            />
          </div>
