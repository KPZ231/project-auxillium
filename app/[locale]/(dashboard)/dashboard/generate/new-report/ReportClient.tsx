...
export default function ReportClient({ spaceId, locale }: ReportClientProps) {
  ...
  if (!spaceId) {
    return (
      <div className="flex h-full items-center justify-center text-[#71717A] text-[14px] p-10">
        {t("dashboard:metrics.report_no_space", "Please select or create a space to view the report generator.")}
      </div>
    );
  }
          ...
          <div className="flex flex-col gap-12 font-sans">
            ...
            <div className="flex justify-between items-end border-b-2 border-[#0A0A0A] pb-6 mb-4">
              ...
              <div className="text-right text-[13px] font-mono text-[#0A0A0A]">
                ...
                <p>{t("dashboard:metrics.report_header_range", "Range")}: {formatDate(data.dateRange.from)} - {formatDate(data.dateRange.to)}</p>
              </div>
            </div>
              ...
              <section className="print:break-inside-avoid">
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="border border-[#0A0A0A] p-4">
                    <p className="text-[12px] uppercase tracking-widest text-[#71717A] mb-1">{t("dashboard:metrics.report_stat_total_expenses", "Total Expenses")}</p>
                    <p className="text-[24px] font-bold">
                      {formatCurrency(data.finances.expenses.reduce((sum: number, e: any) => sum + e.amount, 0))}
                    </p>
                  </div>
                  <div className="border border-[#0A0A0A] p-4">
                    <p className="text-[12px] uppercase tracking-widest text-[#71717A] mb-1">{t("dashboard:metrics.report_stat_total_income", "Total Income")}</p>
                    <p className="text-[24px] font-bold">
                      {formatCurrency(data.finances.incomes.reduce((sum: number, e: any) => sum + e.amount, 0))}
                    </p>
                  </div>
                  <div className="border border-[#0A0A0A] p-4">
                    <p className="text-[12px] uppercase tracking-widest text-[#71717A] mb-1">{t("dashboard:metrics.report_stat_active_projects", "Active Projects")}</p>
                    <p className="text-[24px] font-bold">{data.projects.length}</p>
                  </div>
                  <div className="border border-[#0A0A0A] p-4">
                    <p className="text-[12px] uppercase tracking-widest text-[#71717A] mb-1">{t("dashboard:metrics.report_stat_pending_tasks", "Pending Tasks")}</p>
                  ...
                  <table className="w-full text-left text-[13px] font-mono border-collapse border border-[#0A0A0A]">
                    <thead>
                      <tr className="bg-[#FAFAFA] border-b border-[#0A0A0A]">
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_project_name", "Project Name")}</th>
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_status", "Status")}</th>
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_budget", "Budget")}</th>
                        <th className="p-3 uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_due_date", "Due Date")}</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                <h2 className="text-[20px] font-bold uppercase border-b border-[#D4D4D8] pb-2 mb-4 text-[#0A0A0A]">
                 ...
                 {data.tasks.length === 0 ? (
                  ...
                  <table className="w-full text-left text-[13px] font-mono border-collapse border border-[#0A0A0A]">
                    <thead>
                      <tr className="bg-[#FAFAFA] border-b border-[#0A0A0A]">
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_task_title", "Task Title")}</th>
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_project", "Project")}</th>
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_assignee", "Assignee")}</th>
                        <th className="p-3 border-r border-[#0A0A0A] uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_priority", "Priority")}</th>
                        <th className="p-3 uppercase tracking-wider font-bold">{t("dashboard:metrics.report_col_due_date", "Due Date")}</th>
                      </tr>
                    </thead>
                    <tbody>
            ...
            {sections.finances && (
              <section className="print:break-inside-avoid">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 print:grid-cols-2">
                  {/* Expenses */}
                  <div>
                    <h3 className="text-[16px] font-bold mb-3 uppercase text-[#0A0A0A]">{t("dashboard:metrics.report_title_expenses", "Expenses")}</h3>
                    {data.finances.expenses.length === 0 ? (
                      <p className="text-[14px] text-[#71717A] italic">{t("dashboard:metrics.report_no_expenses", "No expenses in this period.")}</p>
                    ) : (
                      <table className="w-full text-left text-[12px] font-mono border-collapse border border-[#0A0A0A]">
                        <thead>
                          <tr className="bg-[#FAFAFA] border-b border-[#0A0A0A]">
                            <th className="p-2 border-r border-[#0A0A0A]">{t("dashboard:metrics.report_col_date", "Date")}</th>
                            <th className="p-2 border-r border-[#0A0A0A]">{t("dashboard:metrics.report_col_category", "Category")}</th>
                            <th className="p-2 border-r border-[#0A0A0A]">{t("dashboard:metrics.report_col_desc", "Desc")}</th>
                            <th className="p-2 text-right">{t("dashboard:metrics.report_col_amount", "Amount")}</th>
                          </tr>
                        </thead>
                        <tbody>
                  ...
                  {/* Incomes */}
                  <div>
                    <h3 className="text-[16px] font-bold mb-3 uppercase text-[#0A0A0A]">{t("dashboard:metrics.report_title_incomes", "Incomes")}</h3>
                    {data.finances.incomes.length === 0 ? (
                      <p className="text-[14px] text-[#71717A] italic">{t("dashboard:metrics.report_no_incomes", "No incomes in this period.")}</p>
                    ) : (
                      <table className="w-full text-left text-[12px] font-mono border-collapse border border-[#0A0A0A]">
                        <thead>
                          <tr className="bg-[#FAFAFA] border-b border-[#0A0A0A]">
            ...
            {/* Footer */}
             <div className="mt-12 pt-4 border-t border-[#D4D4D8] text-center text-[10px] font-mono text-[#71717A] print:mt-auto">
                {t("dashboard:metrics.report_footer", "Confidential Report")} • {data.space.name} • {t("dashboard:metrics.report_generated_by_system", "Generated by Project Auxillium")}
             </div>
