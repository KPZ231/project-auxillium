...
export async function POST(req: Request) {
  try {
    ...
    if (!features.ai) {
      return new Response(
        JSON.stringify({ error: "AI_NOT_ON_PLAN", plan }),
    ...
    if (plan === "PRO") {
      ...
      if (!success) {
        return new Response(
          JSON.stringify({ error: "AI_RATE_LIMIT_EXCEEDED", reset }),
    ...
    const result = streamText({
      ...
      tools: (() => {
        ...
        const googleTools = features.googleConnectors
          ? {
              ...
              createGoogleCalendarEvent: tool({
                ...
                execute: async (data) =>
                  await createGoogleCalendarEvent(userId, data),
              }),
              ...
              uploadToGoogleDrive: tool({
                ...
                execute: async (data) =>
                  await uploadToGoogleDrive(userId, data),
              }),
            }
          : {};
