...
function ProjectCard({ project }: { project: Project }) {
  ...
  return (
    <Link href={`/dashboard/projects/${project.id}`} className="block group h-full">
      <div className="relative w-full h-full min-h-[200px] bg-[#F4F4F5] border border-[#E5E5E5] group-hover:border-[#0A0A0A] transition-colors duration-200 overflow-hidden flex flex-col">

        {/* Background image - faded/greyscale */}
        {hasImage && (
          <div
        ...
        <div className="relative z-10 flex flex-col justify-between h-full p-5">
          ...
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={(e) => { e.preventDefault(); /* TODO: actions */ }}
              className="w-7 h-7 flex items-center justify-center border border-[#D4D4D8] hover:border-[#0A0A0A] bg-white/80 text-[#0A0A0A] text-xs transition-colors duration-150"
            >
              ···
            </button>
          </div>
          ...
          <div className="mt-auto pt-6">
            {/* Project name */}
            <h3 className="text-[22px] font-black leading-[1.1] tracking-tight text-[#0A0A0A] uppercase mb-1 line-clamp-2">
              {project.projectName}
            </h3>
    ...
    <section className="w-full py-6 px-4">
      ...
      <div className="flex justify-end mb-6">
        <button
          ...
          className="px-4 py-2 text-[12px] uppercase tracking-[0.05em] font-medium border border-[#D4D4D8] hover:border-[#0A0A0A] bg-white text-[#0A0A0A] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
          {isRefreshing ? (
      ...
      {projects.length === 0 ? (
        ...
        <motion.div
          className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-[1px] bg-[#E5E5E5]"
          initial="hidden"
          animate="visible"
          variants={containerVariants}
        >
          {projects.map((project, index) => {
            // Dynamiczne przypisywanie rozmiaru podobnie do pierwotnego mockupa
            ...
            return (
              <motion.div
                ...
                className={`${colSpan} ${rowSpan} bg-white`}
                style={{ minHeight }}
              >
